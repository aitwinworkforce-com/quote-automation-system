...
export async function fetchLiveRates(): Promise<LiveRates> {
  ...
  try {
    const res = await fetch("https://api.frankfurter.dev/v1/latest?base=AUD&symbols=EUR,USD");
    if (res.ok) {
      const data = (await res.json()) as { rates: { EUR: number; USD: number }; date: string };
      return {
        base: "AUD",
  ...
  const data2 = (await res2.json()) as { rates: { EUR: number; USD: number } };
  return {
    base: "AUD",
